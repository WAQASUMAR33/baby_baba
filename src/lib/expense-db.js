...
async function getTableNames() {
  ...
  tableNamesCache = {
    expensetitle: resolve(['ExpenseTitle', 'expensetitle', 'expense_title', 'expense_titles']),
    expense: resolve(['Expense', 'expense', 'expenses']),
...
export async function createExpenseTitle(expTitle) {
  ...
  try {
    const { expensetitle } = await getTableNames()
...
export async function createExpense(expenseData, userId) {
  ...
  try {
    const { expense, expensetitle, user } = await getTableNames()
    const [result] = await connection.execute(
