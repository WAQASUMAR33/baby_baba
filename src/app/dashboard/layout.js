...
export default function DashboardLayout({ children }) {
  ...
  const allNavigation = [
    ...
    },
    {
      ...
      icon: (
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6m4-9a9 9 0 110 18 9 9 0 010-18z" />
        </svg>
      ),
    },
    {
  ...
  const hasModuleRestrictions = userModules.length > 0
  ...
  const currentModule = allNavigation
    .filter((item) => pathname === item.href || pathname.startsWith(`${item.href}/`))
    .sort((a, b) => b.href.length - a.href.length)[0]
  const fallbackHref = navigation[0]?.href

  useEffect(() => {
    if (!mounted) return
    const allowedByRole = !currentModule?.roles || currentModule.roles.includes(userRole)
    const allowedByModule = !hasModuleRestrictions || (currentModule?.moduleKey && userModules.includes(currentModule.moduleKey))
    if (allowedByRole && allowedByModule) return
    if (!fallbackHref || fallbackHref === pathname) return
    router.push(fallbackHref)
  }, [mounted, hasModuleRestrictions, currentModule?.moduleKey, currentModule?.roles, userModules, userRole, router, fallbackHref, pathname])
          ...
          <div className="flex items-center justify-between h-16 px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex items-center space-x-4">
              ...
              <button
                ...
                className="hidden sm:inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-lg text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors"
              >
                Sign out
              </button>
            </div>
